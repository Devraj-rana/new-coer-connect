...
import { createFirebaseQuiz, FirebaseQuizQuestion } from '@/lib/firebase/firestore';
...
export default function FirebaseCreateQuizPage() {
  ...
  // Questions
  const [questions, setQuestions] = useState<FirebaseQuizQuestion[]>([
    {
      question: "",
  ...
  if (!userProfile) {
    return (
      <div className="min-h-screen bg-gray-50 pt-20 flex items-center justify-center">
        <Alert>
          <AlertDescription>Please sign in to create quizzes.</AlertDescription>
        </Alert>
      </div>
    );
  }
  ...
  if (userProfile.role !== 'teacher') {
    return (
      <div className="min-h-screen bg-gray-50 pt-20 flex items-center justify-center">
        <Alert>
          <AlertDescription>Only teachers can create quizzes.</AlertDescription>
        </Alert>
      </div>
    );
  }
  ...
  const handleCreateQuiz = async () => {
    try {
      ...
      const { availableFrom, availableUntil, ...restSettings } = settings;
      const { Timestamp } = await import('firebase/firestore');

      const convertedSettings = {
        ...restSettings,
        availableFrom: availableFrom ? Timestamp.fromDate(availableFrom) : undefined,
        availableUntil: availableUntil ? Timestamp.fromDate(availableUntil) : undefined,
      };

      const result = await createFirebaseQuiz({
        title: title.trim(),
        description: description.trim(),
        classId: classId || undefined,
        questions: validQuestions,
        settings: convertedSettings,
        shareableLink: '', // Will be generated in the function
        isActive: true,
        teacherId: userProfile.uid,
        teacherName: `${userProfile.firstName || ''} ${userProfile.lastName || ''}`.trim(),
  ...
  // ... (rest of the render methods are the same as the original quiz creation page)
  ...
  // Settings and Success components are the same...
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <Button 
            onClick={() => window.open(createdQuiz?.shareableLink, '_blank')} 
            variant="outline"
            className="flex-1"
          >
